    ...
    partial class Program
    {
        ...
        private static void Setup([CanBeNull] string rootDirectory, string[] args)
        {
            ...
            if (rootDirectory == null)
            {
                ...
                ControlFlow.AssertWarn(rootDirectory != null, "Could not find root directory. Falling back to working directory.");
                ...
                else if (definitions.Contains("GIT"))
                {
                    definitions.Add(
                        ConsoleHelper.PromptForChoice("Do you use GitVersion?",
                            ("GIT_VERSION", "Yes, just not setup yet"),
            ...
            if (new[]{"addon", "addin", "plugin"}.Any(x => x.EqualsOrdinalIgnoreCase(args.FirstOrDefault())))
            {
                ...
                var organization = ConsoleHelper.PromptForInput("Organization name:", "nuke-build");
                var addonName = ConsoleHelper.PromptForInput("Organization name:", null);
                var authors = ConsoleHelper.PromptForInput("Author names separated by comma:", "Matthias Koch, Sebastian Karasek");
                var packageName = ConsoleHelper.PromptForInput("Package name on nuget.org:", null);
